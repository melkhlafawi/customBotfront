...
function getNewJwtToken(userId, userName) {
    ...
    const claims = {
        "userId": userId,
        "userName": userName,
        "hashKey": convertToJwtToken('intdvTeam')
    }
    const encodedPlayload = btoa(JSON.stringify(claims))
    const signature = HMACSHA256(`${encodedHeaders}.${encodedPlayload}`, "intdv")
...
function convertToJwtToken(message) {
    const HMACSHA256 = (stringToSign, secret) => "not_implemented"
    const header = {
        "alg": "HS256",
        "typ": "JWT",
    ...
    const encodedHeaders = btoa(JSON.stringify(header))
    const encodedPlayload = btoa(JSON.stringify(message))
    const signature = HMACSHA256(`${encodedHeaders}.${encodedPlayload}`, "intdv")
    const encodedSignature = btoa(signature)
